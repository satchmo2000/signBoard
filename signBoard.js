...
Component({
  ...
  properties: {
    ...
    NeedPreview:{
      type : Boolean,
      observer(oldVal, newVal){
        console.log('NeedPreview=', oldVal);

        this.setData({
          previewflag: oldVal ? 1 : 0
        })
      }
    },
  },
  ...
  ready: function(e){
    ...
    console.log('ctx in component=', ctx)
    ...
    query.select('.handCenter').boundingClientRect( rect => {
      console.log('rect in component=', rect)
      this.setData({
  ...
  methods: {
    ...
    getCanvasId: function(){
      return this.data.canvasName;
    },
    //将签名文件保存并通过回调在外部处理签名文件
    saveCanvasAs(callback){
      let canvasName = this.data.canvasName
    ...
    //上传（代码仅供参考）
    uploadCanvasImg(inputid, canvasName) {
        ...
        success(res) {
          ...
          wx.uploadFile({
            ...
            success(res) {
              ...
              //console.log(data);
              //将data.files中的名称作为签名更新订单的签名信息
              var dataUrl = baseUrl + '/TaskOutputLinkSignFile?tokenid=' + token + '&inputid=' + inputid + '&signfile=' + data.files;
              //console.log(dataUrl);
